...
test_that("Read an element in tensor at index position", {
  ...
  # print(new_tensor)
  expect_equal(new_tensor[0][0]$item(), 1)
  expect_equal(new_tensor[0][1]$item(), 2)
  expect_equal(new_tensor[1][0]$item(), 3)
  expect_equal(new_tensor[1][1]$item(), 4)
})
...
test_that("Proper way to assign a value to a tensor", {
  new_tensor = torch$Tensor(list(list(1, 2), list(3, 4)))
  new_tensor[0][0]$fill_(5)
  new_tensor[0][1]$fill_(6)
  new_tensor[1][0]$fill_(7)
  new_tensor[1][1]$fill_(8)
  expect_equal(new_tensor[0][0]$item(), 5)
  expect_equal(new_tensor[0][1]$item(), 6)
  expect_equal(new_tensor[1][0]$item(), 7)
  expect_equal(new_tensor[1][1]$item(), 8)
})
...
test_that("Write to tensor using Python, declare global", {
  ...
  expect_equal(new_tensor[0][0]$item(), 7)
})
