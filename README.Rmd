...
Besides the module `torch`, which provides `PyTorch` methods, classes and functions, we are also providing numpy as a method `np`, and `torchvision` as well. The dollar sign after the module will provide you access to their objects.
...
Before start running `rTorch`, install a Python Anaconda environment first. Then:
