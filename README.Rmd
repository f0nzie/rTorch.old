...
The goal of `rTorch` is providing an R wrapper to [PyTorch](https://pytorch.org/). `rTorch` provides all the functionality of  PyTorch plus all the features that R provides.  We have borrowed ideas and code used in R [tensorflow](https://github.com/rstudio/tensorflow) to implement `rTorch`.
...
## rTorch Examples
To lighten up the time in building this `rTorch` package, we moved the examples that use tensor operations and neural bnetworks to separate repositories. There are two sets of examples:

* _The rTorch Minimal Book_ at https://f0nzie.github.io/rtorch-minimal-book/
* _rTorch Advanced Examples_ at https://github.com/f0nzie/rTorch.examples/tree/master/packages/rTorch.examples
