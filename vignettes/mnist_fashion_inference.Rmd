...
```{python}
# extract tensor 1x28x28
r.img_pick.shape                       # torch.Size([1, 28, 28])
r.img_pick.view(1, 28, 28).shape       # torch.Size([1, 28, 28])

# remove one dimension converting array to 2D
# r.img_pick.resize_(1, 28, 28).numpy().squeeze().shape  # (28, 28)
img = r.img_pick.resize_(1, 28, 28).numpy().squeeze()
img.shape                    # (28, 28)

# convert to 3D array 1x28x28
img = np.reshape(img, (1, 28, 28))
img.shape

# transpose on dim=1,2
img.transpose((1, 2, 0)).shape  # (28, 28, 1) after reshape(1,28,28)
...
image = std * img + mean
# print(image)
image = np.clip(image, 0, 1)  # Clip (limit) the values in an array.

# print(mean.shape, std.shape)
print(image.shape)

# we need to transform the matrix 28x28 to an array 1x28x28

```

```{r, fig.width=3, fig.height=3}
imshow <- function(img, title=NULL, normalize=TRUE) {
  # print(image$shape)
  # image = image$transpose(c(1L, 2L, 0L))
  # print(image)
  # image <- np$transpose(image)
  # image(image)
  
  # mean = np$array(c(0.485, 0.456, 0.406))
  # std  = np$array(c(0.229, 0.224, 0.225))
...
view_classify <- function(imag, ps, version='Fashion') {
  print(imag$shape)
  imag <- imag$resize_(1L, 28L, 28L) #$numpy()$squeeze()
  # print(imag)
  # imag <- np$squeeze(imag$numpy())
  # imag <- r_to_py(imag)
  # imag <- imag$numpy()
  
  print(class(imag))   # numpy.ndarray
  # print(imag$shape)    # (28,28)
  # imag <- r_to_py(imag$numpy())
  imag <- imag$numpy()
  # imag <- imag
  print(dim(imag))
  ...
  print(class(imag))
  image(imshow(imag))   # image() cannot view multidimensional arrays
}

# ERROR:
# view_classify(img_pick$view(1L, 28L, 28L), ps)
# Error in py_call_impl(callable, dots$args, dots$keywords) : ValueError: operands could not be broadcast together with shapes (3,) (28,28)
```



```{python}
# Python equivalent operations to replicate in R
# plt.imshow() from matplotlib can plot multidmensional arrays
# R image() only plot 2D arrays or matrices
arr = np.arange(784)
arr = np.reshape(arr, (1, 28, 28))
arr.shape

arr = np.transpose(arr, (1,2,0))  # transpose array to multiply by 1x3 or 3x1
arr.shape
