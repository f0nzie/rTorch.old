...
n_classes     <- 10L
batch_size    <- 100L
epochs        <- 5        # original value for epochs = 100
neurons       <- 512L
dropout       <- 0.2
```
...
build_model <- function(input_dim, output_dim) {
    ...
    model$add_module("dropout_1", torch$nn$Dropout(dropout))
    model$add_module("linear_2",  torch$nn$Linear(neurons, neurons, bias = FALSE))
    model$add_module("relu_2",    torch$nn$ReLU())
    model$add_module("dropout_2", torch$nn$Dropout(dropout))
    model$add_module("linear_3",  torch$nn$Linear(neurons, output_dim, bias = FALSE))
    return(model)
}
...
# Epochs
# Epoch =   1, cost = 0.284543, acc = 95.47% 
